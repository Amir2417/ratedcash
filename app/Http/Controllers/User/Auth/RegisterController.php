...
class RegisterController extends Controller
{
    ...
    */
    ...
    public function sendVerifyCode(Request $request){
        ...
        $exist = User::where($field_name,$validated['mobile'])->first();
        if( $exist) return back()->with(['error' => ['User already  exists, please try with another mobile number']]);
         ...
         if( $basic_settings->sms_verification == false){
            ...
            return redirect()->route("user.register.kyc")->with(['success' => ['Please fillup required field for registration']]);
        ...
        return redirect()->route('user.sms.verify',$data['token'])->with(['success' => ['Verification code send to your mobile number successfully!']]);
    }
    /**
     * Method for sms verify
     */
    public function smsVerify($token)
    {
        $page_title = "Mobile Verification";
        $data = UserAuthorization::where('token',$token)->first();
        return view('user.auth.authorize.verify-sms',compact("page_title","data","token"));
    }
    /**
