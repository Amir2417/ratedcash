            ...
            <div class="dash-payment-item-wrapper">
                <div class="dash-payment-item active">
                    ...
                    <div class="dash-payment-body">
                        <form class="card-form" action="{{ setRoute('user.receipient.update',$data->id) }}" method="POST">
                            @csrf
                            @method("PUT")
                            <div class="row">
                                <div class="col-xl-12 col-lg-12 form-group transaction-type">
                                    ...
                                    <select name="bank_name" class="form--control select2-auto-tokenize" id="receipent_select" required data-placeholder="Select Bank" >
                                        ...
                                        @foreach ($banks ?? [] as $bank)
                                            <option value="{{ $bank['code'] }}|{{ $bank['name'] }}" {{ $bank['code'] == $data->bank_code ?'selected' : '' }}>{{ $bank['name'] }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="col-lg-12 form-group">
                                    <label for="account_number">{{ __("Account Number") }} <span class="text-danger">*</span></label>
                                    <input type="number" class="form--control " id="account_number"  name="account_number" value="{{ old('account_number',$data->account_number) }}" placeholder="Account Number" required>
